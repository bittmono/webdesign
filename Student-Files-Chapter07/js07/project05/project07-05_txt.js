...
/*    JavaScript 7th Edition
      ...
      notes:
      - file -> files for both anon functions?
...
function generateWordFreq(inputFile, outputDoc, outputCount) {
   ...
   let fr = new FileReader(); //changed from Reader to FileReader
   fr.readAsText(inputFile); //read to readAsText
   ...
   fr.onload=function() { 
      ...
      let sourceText = outputDoc.textContent; //change from innerHTML to textCont
      ...
      let words = sourceText.split(/\s+/); // Added '+' after '\s' 
      ...
      for (let i = 0; i < words.length; i++) {    //changed <= to <
      ...
      for (let i = 1; i <= 15; i++) {
         ...
         let percent = ((freqs[i] / totalWords) * 100).toFixed(1) + "%"; //changed calculation
